	...
	internal class BaseModuleEditor : Editor
	{
		...
		private BaseModule.Attribute AttributeDrawer(Rect position, BaseModule.Attribute attribute)
		{
			...
			else
			{
				if (attribute.type == BaseModule.Attribute.AttributeType.FREQUENCY)
				{
                    if (attribute.generator == null)
                    {
                        ...
                        for (int i = 1; i < 7; i++)
                        {
                            for (int j = 0; j < 12; j++)
                            {
                                list.Add(i.ToString() + " - " + global::DarkArtsStudios.SoundGenerator.DarkArtsStudios
                                             .Audio.Music.NoteName[j]);
                                if (attribute.value >= global::DarkArtsStudios.SoundGenerator.DarkArtsStudios.Audio
                                        .Music.Frequency(i, j + 1))
                                {
                                    num = num2;
                                }
                        ...
                        int octaveNote =
                            global::DarkArtsStudios.SoundGenerator.DarkArtsStudios.Audio.Music.OctaveNote(
                                attribute.value);
                        int selectedValue =
                        ...
                        selectedValue = EditorGUI.IntPopup(
                            new Rect(position.x + position.width / 2f, position.y, position.width / 2f,
                                AttributeHeight), selectedValue, new string[8]
                            {
                                "1",
                                "2",
                                "3",
                        ...
                        selectedIndex =
                            EditorGUI.Popup(new Rect(position.x, position.y, position.width / 2f, AttributeHeight),
                                selectedIndex,
                                global::DarkArtsStudios.SoundGenerator.DarkArtsStudios.Audio.Music.NoteName.ToArray());
                        if (EditorGUI.EndChangeCheck())
                        {
                            global::DarkArtsStudios.SoundGenerator.DarkArtsStudios.Audio.Music.Octave(octaveNote);
                            global::DarkArtsStudios.SoundGenerator.DarkArtsStudios.Audio.Music.Note(octaveNote);
                            float num3 = attribute.value =
                                global::DarkArtsStudios.SoundGenerator.DarkArtsStudios.Audio.Music.Frequency(
                        ...
                        attribute.value =
                            EditorGUI.FloatField(
                                new Rect(position.x, position.y + AttributeHeight, position.width, AttributeHeight),
                ...
                else if (attribute.type == BaseModule.Attribute.AttributeType.BUTTON)
                {
                    //EditorGUI.BeginChangeCheck();
                    ...
                    attribute.pressed = EditorGUI.Toggle(position2, attribute.pressed);
                    //EditorGUI.EndChangeCheck();
                }
                else if (attribute.type == BaseModule.Attribute.AttributeType.SLIDER)
                {
                    if (attribute.generator == null)
                    {
                        Rect position2 = EditorGUI.PrefixLabel(position, 0,
                            global::DarkArtsStudios.SoundGenerator.DarkArtsStudios.Core.GUIUtility.TempContent(
                                attribute.name));
                        attribute.value = EditorGUI.Slider(position2, attribute.value, attribute.clampMinimumValue,
                            attribute.clampMaximumValue);
                    }
                    else
                    {
                        EditorGUI.LabelField(position, attribute.name);
