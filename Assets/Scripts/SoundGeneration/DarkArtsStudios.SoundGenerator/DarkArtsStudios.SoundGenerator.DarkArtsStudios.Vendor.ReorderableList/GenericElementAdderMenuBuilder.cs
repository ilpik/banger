...
namespace DarkArtsStudios.SoundGenerator.DarkArtsStudios.Vendor.ReorderableList
{
	internal sealed class GenericElementAdderMenuBuilder<TContext> : IElementAdderMenuBuilder<TContext>
	{
		...
		{
			_typeDisplayNameFormatter = (formatter ?? new Func<Type, string>(NicifyTypeName));
		}
		...
		{
			GenericElementAdderMenu genericElementAdderMenu = new GenericElementAdderMenu();
			AddCommandsToMenu(genericElementAdderMenu, _customCommands);
			if (_contractType != null)
			{
				AddCommandsToMenu(genericElementAdderMenu, ElementAdderMeta.GetMenuCommands<TContext>(_contractType));
				AddConcreteTypesToMenu(genericElementAdderMenu, ElementAdderMeta.GetConcreteElementTypes(_contractType, _typeFilters.ToArray()));
			}
			return genericElementAdderMenu;
		}
		...
		private void AddCommandsToMenu(GenericElementAdderMenu menu, IList<IElementAdderMenuCommand<TContext>> commands)
		{
			if (commands.Count != 0)
			{
				...
				foreach (IElementAdderMenuCommand<TContext> command in commands)
				{
					...
					{
						menu.AddItem(command.Content, delegate
						{
							command.Execute(_elementAdder);
						});
					}
					else
					{
