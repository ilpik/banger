	...
	internal class Fourier
	{
		...
		public static void FFT(ComplexF[] data, int length, FourierDirection direction)
		{
			...
			for (int i = 1; i <= num; i++)
			{
				...
				for (int j = 0; j < num4; j++)
				{
					...
					{
						int num7 = k + num4;
						float re = data[num7].Re;
						float im = data[num7].Im;
						float num8 = re * num5 - im * num6;
						float num9 = re * num6 + im * num5;
						re = data[k].Re;
						im = data[k].Im;
						data[k].Re = re + num8;
						data[k].Im = im + num9;
						data[num7].Re = re - num8;
						data[num7].Im = im - num9;
					}
				}
			}
		}
		...
		public static void FFT(Complex[] data, int length, FourierDirection direction)
		{
			...
			for (int i = 1; i <= num; i++)
			{
				int num4 = num2;
				num2 <<= 1;
				double[] array = _uRLookup[i, num3];
				double[] array2 = _uILookup[i, num3];
				for (int j = 0; j < num4; j++)
				{
					double num5 = array[j];
					...
					{
						int num7 = k + num4;
						double re = data[num7].Re;
						double im = data[num7].Im;
						double num8 = re * num5 - im * num6;
						double num9 = re * num6 + im * num5;
						re = data[k].Re;
						im = data[k].Im;
						data[k].Re = re + num8;
						data[k].Im = im + num9;
		...
		public static void FFT3(ComplexF[] data, int xLength, int yLength, int zLength, FourierDirection direction)
		{
			...
			for (int m = 0; m < yLength; m++)
			{
				...
				{
					int start3 = m * xLength + n * num;
					LinearFFT_Quick(data, start3, num2, zLength, direction);
				}
			}
		}
